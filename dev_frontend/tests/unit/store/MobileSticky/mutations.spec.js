import mutations from '../../../../src/store/modules/MobileSticky/MobileStickyMutations'

describe('MobileStickyMUTATIONS module:', () => {
  ...
  test('CLEAR_INACTIVE_ITEMS sets state.inactiveItems to an empty array', () => {
    const state = {
      inactiveItems: [ 'some shit', 'other shit', 'joker' ]
    }

    mutations.CLEAR_INACTIVE_ITEMS( state )
    expect( state.inactiveItems ).toEqual( [] )
  })

  test('PUT_TO_INACTIVE pushes item to inactive items', () => {
    const newItems = [{ item1: 'test1' }, { item2: 'test2' }, { item3: 'test3' }]
    const state = {
      inactiveItems: []
    }

    mutations.PUT_TO_INACTIVE( state, { item: newItems[ 0 ] })
    expect( state.inactiveItems.length ).toBe( 1 )

    mutations.PUT_TO_INACTIVE( state, { item: newItems[ 1 ] })
    mutations.PUT_TO_INACTIVE( state, { item: newItems[ 2 ] })
    expect( state.inactiveItems ).toEqual( newItems )
  })

  test('REMOVE_ITEM_FROM_INACTIVE remove object at given index from state.inactiveItems', () => {
    expect.assertions( 2 )
    ...
    mutations.REMOVE_ITEM_FROM_INACTIVE( state, { cellId: 2 })
    expect( state.inactiveItems.length ).toBe( 4 )
    expect( state.inactiveItems ).not.toContainEqual( { "name": "menuTEST", "icon": "list", "form": { "options": { "activeForSubmenuItems": false, "bordersBetweenItems": false, "arrowsForItemsWithSubmenu": false, "foldSubmenusForLast2ItemsToRight": false }, "replaceWithMenuIcon": "tabletMobile", "menu": "2" }, "originalCoords": { "row": "firstRow", "cellPos": "left", "cellId": 2 } } )
  })
